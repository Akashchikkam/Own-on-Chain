...
function DistributorDashboard() {
  ...
  useEffect(() => {
    if (isConnected && account && provider) {
      ...
      const timer = setTimeout(() => {
        ...
        loadProducts(); // This will check role and only load if verified Distributor
      }, 150);
  ...
  const checkDistributorRole = async () => {
    ...
    try {
      ...
      const hasDistributorRole = await participantRegistryService.hasRole(provider, account, Role.DISTRIBUTOR);
      ...
      if (!hasDistributorRole) {
        ...
        try {
          ...
          if (Number(participant.role) === Role.DISTRIBUTOR && Number(participant.status) !== 1) {
            setRoleError(`⚠️ Your account is registered as Distributor but NOT VERIFIED yet. Please wait for admin verification.`);
          } else {
            setRoleError(`❌ You are registered as ${currentRole}, not Distributor. Switch to Distributor account to view inventory.`);
          }
          ...
          setRoleError(`❌ This account is not registered as Distributor. Please register as Distributor or switch to Distributor account.`);
          setAccountInfo({
            address: account,
            role: 'Not Registered',
            status: null,
        ...
        try {
          ...
          setAccountInfo({
            address: account,
            role: RoleName[Number(participant.role)] || 'Distributor',
            status: participant.status,
            isDistributor: true
          });
        } catch (err) {
  ...
  const loadProducts = async () => {
    ...
    // CRITICAL: Only load products if user is verified Distributor
    // This prevents products showing in wrong dashboards
    try {
      const { participantRegistryService } = await import('../utils/contractHelpers');
      const { Role } = await import('../contracts/config');
      const hasDistributorRole = await participantRegistryService.hasRole(provider, currentAccount, Role.DISTRIBUTOR);
      if (!hasDistributorRole) {
        console.log('⚠️ Not a verified Distributor - skipping product load');
        setProducts([]);
        setLoading(false);
        return;
      }
    } catch (roleCheckErr) {
      console.warn('⚠️ Could not verify Distributor role - skipping product load:', roleCheckErr);
      setProducts([]);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log('🔍 Loading products for Distributor:', currentAccount);
        ...
        tokenIds.map(async (tokenId) => {
          try {
            ...
            const product = {
              ...
              status: supplyChainData ? Number(supplyChainData.status) : (productInfo.isActive ? 1 : 0),
    ...
    try {
      ...
      try {
        if (recipientRole === Role.RETAILER) {
          ...
          setSuccess(`Product #${tokenId} transferred to Retailer successfully!`);
          ...
          // Other roles - use direct NFT transfer (still tracked on blockchain via NFT transfer history)
  ...
  return (
    <div className="dashboard">
      <div className="dashboard-header">
        ...
        <p>Manage and forward products to retailers</p>
      </div>
      ...
      {accountInfo && (
        <div style={{ 
          padding: '0.75rem', 
          marginBottom: '1rem', 
          backgroundColor: accountInfo.isDistributor ? '#d4edda' : '#f8d7da',
          border: `1px solid ${accountInfo.isDistributor ? '#c3e6cb' : '#f5c6cb'}`,
          borderRadius: '4px',
          fontSize: '0.9rem'
        }}>
          ...
          {!accountInfo.isDistributor && (
            <>
              <br/>
              <span style={{ color: '#721c24', fontWeight: 'bold' }}>
                ❌ This is NOT the Distributor account! Switch to Distributor account to view inventory.
              </span>
            </>
          )}
        </div>
      ...
      <div className="card">
        <h2>My Inventory ({products.length})</h2>
        {loading ? (
            ...
            {products.map((product) => (
              <div key={product.tokenId} className="product-card card">
                ...
                <div className="product-meta">
                  <span className="badge badge-warning">In Inventory</span>
                </div>
